...
(function () {
    ...
    this.registerSystem.push({
        ...
        onInit: function () {
            ...
            this.listen = function (aCanvas) {
                aCanvas.addEventListener('touchstart', function (event) {
                    if (engine.input) {
                ...
                aCanvas.addEventListener('touchmove', function (event) {
                ...
                aCanvas.addEventListener('touchend', function (event) {
                    if (engine.input) {
                        engine.input.blur();
                        engine.input = null;
                        return
            ...
            if (this.touchEvent.length > 0) {
                ...
                for (var j = 0; j < this.touchEvent.length; j++) {
                    ...
                    for (var i = 0; i < renderOrder.length; i++) {
                        ...
                        if (tComponent) {
                            var tRenderProperty = tComponent.entity.components.render.property;
                            var tListener = tComponent.property.callback;
                            var comNode = tComponent.entity.components.node;
                            var dx = comNode.property.width * comNode.property.anchorPointX * tRenderProperty.scaleX;
                            var dy = comNode.property.height * comNode.property.anchorPointY * tRenderProperty.scaleY;
                            var x = tEvent.x - tRenderProperty.x + dx;
                            var y = tEvent.y - tRenderProperty.y + dy;
                            var localX = x / tRenderProperty.scaleX;
                            var localY = y / tRenderProperty.scaleY;

                            //console.log(tEvent.x, tEvent.y);
                            //console.log(localX, localY);
                            //console.log(tComponent.property.width, tComponent.property.height);
